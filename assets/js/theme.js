...
function isMobileWindow () {
  return window.matchMedia('only screen and (max-width: 450px)').matches
}
...
function initSearch () {
  ...
  const isMobile = window.matchMedia && window.matchMedia('only screen and (max-width: 450px)').matches
  ...
  const initAutosearch = () => {
    const autosearch = autocomplete(`#search-input-${suffix}`, {
        ...
        if (searchConfig.type === 'lunr') {
          const search = () => {
            ...
            window._index.search(query).forEach(({ ref, matchData: { metadata } }) => {
              ...
              let { url, title, content: context } = matchData
          ...
          if (!window._index) {
            ...
            fetch( searchConfig.algoliaIndex+query)
              .then(response => response.json())
              .then(data => {
      ...
      templates: {
        suggestion: ({ title, date, context,url }) => `<div><span class="suggestion-title"><a href=${url} target="_blank" title=${title} rel="noopener noreferrer" >${title}</a></span><span class="suggestion-date">${date}</span></div><div class="suggestion-context">${(context)}</div>`,
        ...
        footer: ({ query }) => {
          const { searchType, icon, href } = searchConfig.type === 'algolia'
            ? {
                ...
                href: 'https://www.google.com/search?q=site:commiex.com%20'+ `${escape(query)}`
              }
          ...
          return `<div class="search-footer">Go Search <a href="${href}" rel="noopener noreffer" target="_blank">${icon} ${searchType}</a></div>`
        }
      }
    })
...
function initToc () {
  ...
  if (tocCore === null || document.getElementById('toc-auto') ===null ) return
  const isTocStatic = window.matchMedia && window.matchMedia('only screen and (max-width: 740px)').matches
  ...
  if (document.getElementById('toc-static').getAttribute('kept') || isTocStatic) {
    ...
    window._tocOnScroll = window._tocOnScroll || (() => {
      ...
      if (window.newScrollTop < minScrollTop) {
		...
		if (rel != null) {rel.style.top = `${minTocTop}px`}
		...
		if (rel != null) {rel.style.top = `${maxTocTop}px`}
      } else {
        // If in the middle
        // Set toc to fixed with TOP_SPACING
        toc.style.position = 'fixed'
        toc.style.top = `${TOP_SPACING}px`
		if (rel != null) {rel.style.position = 'fixed'
		rel.style.top = `${TOP_SPACING}px`}
      }
...
function onScroll () {
  ...
  const ACCURACY = 25; const MINIMUM = 100
...
// document.addEventListener('pjax:send', console.log("send"))
...
window.onload = function() {
  ...
  document.addEventListener('visibilitychange', function() {
    if(document.hidden) {
     	var kid = document.querySelector(".ripple");
		if (kid != null) {
		var parent = kid.parentElement;
		parent.removeChild(kid);		
		};
      document.title = 'busy elsewhere?';
...
function ripple(event, target) {
    ...
    function end() {
		console.log(endType)
		document.body.removeEventListener(endType, end);
		document.body.removeEventListener('pjax:start', end);
    ...
    document.body.addEventListener(endType, end);
	// document.body.addEventListener('pjax:start', console.log("start"))
	// document.body.addEventListener('pjax:beforeReplace', console.log("beforeReplace"));
	// document.body.addEventListener('pjax:beforesend', console.log("beforesend"));
	
	// document.body.addEventListener('pjax:popstate', console.log("beforpop"));
...
/** 添加事件类型 */
// const startType = mobile ? "touchstart" : "mousedown";
const startType = "touchstart";
// const endType = mobile ? "touchend" : "mouseup";
const endType = "touchend";


document.body.addEventListener(startType, function (e) {
    /** 事件类型 */
    const event = e || window.event || arguments.callee.caller.arguments[0];
    /** 循环的次数 */
    let loopCount = 5; // 这里的 3 次是布局的子节点层数，可根据布局层数增加减少
    /** 
     * 定义目标变量 
     * @type {HTMLElement} 
     */
